/*
    Database-first update for bracket/playoff match slot sources.

    Base schema: hanaka.sql exported on 2026-05-28.
    Target table: dbo.TournamentGroupMatches.

    Run this script on SQL Server after backing up the database.
*/
...
BEGIN TRY
    ...
    IF EXISTS (
        ...
        FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.TournamentGroupMatches')
          AND name = N'Team1SourceType'
          AND (is_nullable = 1 OR max_length <> 30)
    )
        EXEC(N'
            ALTER TABLE [dbo].[TournamentGroupMatches]
                ALTER COLUMN [Team1SourceType] VARCHAR(30) NOT NULL;
        ');
    ...
    IF EXISTS (
        ...
        FROM sys.columns
        WHERE object_id = OBJECT_ID(N'dbo.TournamentGroupMatches')
          AND name = N'Team2SourceType'
          AND (is_nullable = 1 OR max_length <> 30)
    )
        EXEC(N'
            ALTER TABLE [dbo].[TournamentGroupMatches]
                ALTER COLUMN [Team2SourceType] VARCHAR(30) NOT NULL;
        ');
